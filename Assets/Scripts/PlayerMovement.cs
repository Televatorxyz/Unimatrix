...
public class PlayerMovement : MonoBehaviour
{
    ...
    [SerializeField] InputAction boostControl;
    [SerializeField] InputAction breakControl;
    [SerializeField] InputAction RollLControl;
    [SerializeField] InputAction RollRControl;
    [SerializeField] InputAction shootControl;
    ...
    void OnDisable()
    {
        ...
        breakControl.Disable();
        RollLControl.Disable();
        RollRControl.Disable();
    }
    ...
    void Update()
    {
        ...
        float v = -movement.ReadValue<Vector2>().y; // "-" for inverse control
        ...
        if (boostControl.ReadValue<float>() > 0f)  //(Input.GetButtonDown("Action"))
            Boost(true);
        else Boost(false);

        if (breakControl.ReadValue<float>() > 0f)//(Input.GetButtonDown("Fire3"))
            Break(true);
        else Break(false);

        if (RollLControl.ReadValue<float>() > 0f || (RollRControl.ReadValue<float>() > 0f)) //(Input.GetButtonDown("TriggerL") || Input.GetButtonDown("TriggerR"))
        {
           // int dir = RollLControl.ReadValue<float>() ? -1 : 1; //Input.GetButtonDown("TriggerL") ? -1 : 1;
            if (RollLControl.ReadValue<float>() > 0f)
            QuickSpin(-1);
            else QuickSpin(1);
        }
    ...
    /*
    bool IsPressed(this InputAction inputAction) { }
    bool WasPressedThisFrame(this InputAction inputAction) { }
    bool WasReleasedThisFrame(this InputAction inputAction) { }
    */
